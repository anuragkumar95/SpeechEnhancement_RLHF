...
class ComputeScore:
    ...
    def get_scores(self, clips, desired_fs=16000, gpu_id=None):
        mos = []
...
def main(args):
    ...
    audio_clips_list = []
    p808_model_path = os.path.join(args.model_pt, 'model_v8.onnx')

    if args.personalized_MOS:
        primary_model_path = os.path.join('pDNSMOS', 'sig_bak_ovr.onnx')
    else:
        primary_model_path = os.path.join(args.model_pt, 'sig_bak_ovr.onnx')
...
if __name__=="__main__":
    ...
    parser.add_argument('-m', "--model_pt", type=str, default='DNSMOS', 
                        help='Path to DNSMOS model')
