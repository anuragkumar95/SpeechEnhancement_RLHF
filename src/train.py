...
def args():
    ...
    parser.add_argument("--gamma", type=int, default=0.99, help="Reward discount factor")
    parser.add_argument("--tau", type=int, default=0.99, help="target critic soft update factor")
...
class DDPGTrainer:
    def __init__(self, train_ds, test_ds, gpu_id: int):
        ...
        self.actor = TSCNet(num_channel=64, num_features=self.n_fft // 2 + 1)
        self.target_actor = TSCNet(num_channel=64, num_features=self.n_fft // 2 + 1)
    ...
    def train_one_episode(self, batch, args):
        ...
        for step in range(env.steps):
            ...
            next_applied_state = env.get_next_state(state=experience['next_state'],
                                                    action=next_action,
                                                    t=next_t)
